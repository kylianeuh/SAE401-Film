...
export default function GameFooter({
 ...
 {/* 1. La Date de sortie gérée pour les deux modes */}
<div>
  <span className="font-limelight text-xs uppercase tracking-wider text-fb-dark block opacity-80">
    Date de sortie
  </span>
  <span className="text-fb-dark text-base">
    {/* Si on cherche la date complète, on cache tout. Sinon, on affiche le jour/mois, et on cache l'année si on cherche l'année. */}
    {type === "date" 
      ? "??/??/????" 
      : `${new Date(activeCard.date_de_sortie).toLocaleDateString("fr-FR", { day: "numeric", month: "long" })} ${type === "annee" ? "????" : new Date(activeCard.date_de_sortie).getFullYear()}`
    }
  </span>
</div>

{/* 2. La rubrique Genres (Catégories) */}
<div>
  <span className="font-limelight text-xs uppercase tracking-wider text-fb-dark block opacity-80">
    Genres
  </span>
  <span className="text-fb-dark text-base">
    {/* On utilise .join(", ") pour transformer le tableau ["Action", "Fantastique"] en texte "Action, Fantastique" */}
    {activeCard.genres && activeCard.genres.length > 0 
      ? activeCard.genres.join(", ") 
      : "Non renseigné"}
  </span>
</div>
            <div>
              ...
              <span className="text-fb-dark text-base">
                {type === "notes" ? "???" : activeCard.note_globale}
              </span>
            </div>
            <div>
              <span className="font-limelight text-xs uppercase tracking-wider text-fb-dark block opacity-80">
              ...
              <span className="text-fb-dark text-base">
                {type === "duree"
                  ? "???"
                  : `${activeCard.duree_en_minutes} minutes`}
              </span>
            </div>
            <div>
              <span className="font-limelight text-xs uppercase tracking-wider text-fb-dark block opacity-80">
      ...
      <div className="md:col-span-3 flex justify-center items-center pt-2">
        <div
          ...
          className={`cursor-grab active:cursor-grabbing touch-none transition-all duration-200 transform ${
      ...
      <div className="md:col-span-2 flex flex-col justify-start items-center gap-8 pt-6 select-none h-full">
        ...
        <div className="text-center flex flex-col items-center group">
            ...
            className={`w-14 h-10 bg-fb-dark text-fb-cream flex items-center justify-center border-x-4 border-dashed border-fb-cream/20 rounded-sm shadow-sm mb-2 transition-transform active:scale-95
              ${
                hintsCount > 0 && !currentHint
                  ? "cursor-pointer hover:bg-fb-red"
          ...
          <p className="text-sm md:text-base font-black text-fb-dark tracking-wide leading-tight">
            ...
            <span className="font-normal lowercase text-xs md:text-sm block mt-0.5">
              disponibles
            </span>
          </p>
        </div>
        ...
        <div className="text-center flex flex-col items-center">
          <div className="w-14 h-10 bg-fb-dark text-fb-cream flex items-center justify-center border-x-4 border-dashed border-fb-cream/20 rounded-sm shadow-sm mb-2">
            ...
            <svg
              className="w-5 h-5 text-fb-cream"
              viewBox="0 0 24 24"
              fill="currentColor"
            >
